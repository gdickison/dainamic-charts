const CexFormInputs = ({
  ...
  monthOptions,
  regionOptions,
  handleSelectedRegionsChange,
  selectedRegions,
  removeRegion,
  dateRange,
  getData,
  showChangeOptionsButton,
            ...
            <form className="flex-col w-[60%] min-w-[60%] space-y-4" action="#">
              ...
              <div className="pr-4 space-y-4">
                ...
                <select className="w-full mx-2 text-left px-2 border-2 border-blue-400 bg-white rounded-md text-[1.7vw] 3xl:text-3xl" id="msaCode" name="msaCode" defaultValue="" onChange={handleSelectedRegionsChange}>
                  <option disabled></option>
                  {regionOptions && regionOptions.map(singleRegion => {
                    return (
                      <option key={singleRegion.region} value={singleRegion.region} data-display={`${singleRegion.region_name}`}>{singleRegion.region_name}</option>
                    )
                  })}
                </select>
              </div>
